...
* Added a small **splash message** when VScaler is loaded (disabled on Linux) - _easier to verify Opti loaded successfully, can be disabled in the INI_
* Some Overlay fixes
...
* VScaler now checks the local folder first for `amdxcffx64.dll`
...
* Batch file now tries to remove all files & folders of VScaler
...
* VScaler is now skipping format transfer for FSR-FG supported hudless formats
...
## v0.7.0-pre83
* Added VScaler Setup.bat file for auto installation (thanks to **FakeMichau**)
...
* XeSS inputs support. Now if a game supports XeSS upscaling, VScaler can use XeSS inputs instead of DLSS. Sadly, **Unreal XeSS Plugin does not provide a depth buffer which leads degraded visual quality.**  
...
* In-game menu is now always available when VScaler is loaded
...
* Changed XeSS & FSR3.1 load order to first check VScaler and ini folder

**Fixed**
* For scaling ratios below 1.0 DLSS and XeSS can't handle downscaling, now VScaler auto-enable `Output Scaling` with 1.0 ratio when needed (thanks to **Merlinch** & **DARKERthanDA**)
* VScaler wasn't saving `Force Anisotropic Filtering` value #26 (thanks to **aufkrawall**)
...
* Added `SkipFirstFrames` option to `nvngx.ini` which would allow VScaler to skip rendering desired amount of frames (Needed for **Dragons Dogma 2**) (thanks to **CAPCOM**)
...
* Improved DXGI spoofing when VScaler is not `dxgi.dll` (thanks to **Sildur**)
...
* Possible crash during resolution/quality change [issue #11](https://github.com/CRTYPUBG/vscaler/issues/11) (thanks to **donizettilorenzo**)
...
* Automatically redirect calls for features not supported by VScaler to original nvngx on Nvidia GPUs.
...
**Fixed**
* Prevented capturing mouse and keyboard when VScaler is not active
...
## v0.5.5 (Release)
**Added**
* Added DLSS upscaling support, when using DLSS with VScaler you will have DLSS specific;
...
* Also with DLSS you can use VScaler's;
  - Scaling overrides
...
Changed
* Added new options for Dx11wDx12 creation & syncing. I have tried to explain it [here](https://github.com/CRTYPUBG/vscaler/blob/master/Config.md#dx11withdx12-sync-settings). These settings are game & hw dependant so users need to tweak and find best performing configuration for their system. I have set safe and balanced values as default values.


## v0.4.2 (Release)
New:
* XeSS now have a new parameter `CreateHeaps` (default **true**). When it's enabled VScaler internally create heap objects and bind them instead of relying XeSS to create them. This fixes Guardians of the Galaxy black screen with XeSS issue. (Thanks to **TheRazerMD**)

# VScaler 

## v0.4.1 (Release)
Mod renamed to **VScaler**
...
* Added documentation for [settings](https://github.com/VScaler/OmniScaler/blob/master/Config.md) and [issues](https://github.com/VScaler/OmniScaler/blob/master/Config.md) 
