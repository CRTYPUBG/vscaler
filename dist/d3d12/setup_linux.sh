...
show_help() {
    ...
    echo ""
    echo "Options:"
    echo "  --filename=<string>        Set target filename for VScaler.dll. Options are:"
    echo "                            - dxgi.dll"
    echo "                            - winmm.dll"
    echo "                            - version.dll"
    echo "                            - dbghelp.dll"
...
VScaler_FILE="$SCRIPT_DIR/VScaler.dll"

# Remove junk files
rm -f "$SCRIPT_DIR/!! EXTRACT ALL FILES TO GAME FOLDER !!" 2>/dev/null
rm -f "$SCRIPT_DIR/setup_windows.bat" 2>/dev/null

# Check if VScaler.dll exists
if [ ! -f "VScaler.dll" ]; then
    echo "VScaler \"VScaler.dll\" file is not found!"
    echo "Please make sure you extracted all VScaler files to the game folder."
    echo ""
...
select_filename() {
    ...
    if [ -n "$selected_filename" ]; then
        case "$selected_filename" in
            dxgi.dll|winmm.dll|version.dll|dbghelp.dll|d3d12.dll|wininet.dll|winhttp.dll|VScaler.asi)
                return
                ;;
            *)
    ...
    while true; do
        echo ""
        echo "Choose a filename for VScaler (default is dxgi.dll):"
        echo " [1] dxgi.dll"
        echo " [2] winmm.dll"
        echo " [3] version.dll"
        echo " [4] dbghelp.dll"
...
echo "Renaming VScaler file to $selected_filename..."
if ! mv "$VScaler_FILE" "$selected_filename"; then
    echo ""
    echo "ERROR: Failed to rename VScaler file to $selected_filename."
...
if [ -z "$remove_choice" ]; then
    read -p "Do you want to remove VScaler? [y/n]: " remove_choice
fi

if [ "$remove_choice" = "y" ] || [ "$remove_choice" = "Y" ]; then
    echo ""
    echo "Removing VScaler files..."
    
    # Remove VScaler files
    rm -f VScaler.log
    rm -f VScaler.ini
...
EOF
    ...
    sed -i "s/SELECTED_FILENAME_PLACEHOLDER/$selected_filename/g" "remove_VScaler.sh"
    
    # Make the uninstaller executable
    chmod +x "remove_VScaler.sh"
    
    echo ""
    echo "Uninstaller created: remove_VScaler.sh"
    echo ""
}
...
echo ""
echo "Remember: Insert key opens VScaler overlay, Page Up/Down for performance stats"
echo ""
