...
##### Usage 
Just put `nvapi64.dll` next to VScaler and set `OverrideNvapiDll=true` from `VScaler.ini`. This only works when VScaler is working as non-nvngx (not as `nvngx.dll`).

For using without VScaler:  
...
### DXGI
VScaler have built-in DXGI spoofing option which is enabled by default when working as non-nvngx (not as `nvngx.dll`).  
...
### Vulkan
VScaler have built-in Vulkan spoofing options when working as non-nvngx (not as `nvngx.dll`).  
Vulkan spoofing by default disabled and should be enabled from `VScaler.ini` when needed.
```ini
