    ...
    if (Config::Instance()->FsrAgilitySDKUpgrade.value_or_default())
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            if (!pathSet)
            {
                ...
                widePath = widePath + L"\\D3D12_VScaler\\";
...
static void CheckWorkingMode()
{
    ...
    do
    {
        ...
        if (lCaseFilename == "version.dll")
        {
            do
            {
                ...
                {
                    if (!_passThruMode)
                        LOG_INFO("VScaler working as version.dll, original dll loaded from plugin folder");
                ...
                {
                    if (!_passThruMode)
                        LOG_INFO("VScaler working as version.dll, version-original.dll loaded");
                ...
                if (originalModule != nullptr && !_passThruMode)
                    LOG_INFO("VScaler working as version.dll, system dll loaded");
            ...
            {
                if (!_passThruMode)
                    LOG_ERROR("VScaler can't find original version.dll!");
            }
        ...
        if (lCaseFilename == "winmm.dll")
        {
            do
            {
                ...
                {
                    if (!_passThruMode)
                        LOG_INFO("VScaler working as winmm.dll, original dll loaded from plugin folder");
                ...
                {
                    if (!_passThruMode)
                        LOG_INFO("VScaler working as winmm.dll, winmm-original.dll loaded");
                ...
                if (originalModule != nullptr && !_passThruMode)
                    LOG_INFO("VScaler working as winmm.dll, system dll loaded");
            ...
            {
                if (!_passThruMode)
                    LOG_ERROR("VScaler can't find original winmm.dll!");
            }
        ...
        if (lCaseFilename == "wininet.dll")
        {
            do
            {
                ...
                {
                    if (!_passThruMode)
                        LOG_INFO("VScaler working as wininet.dll, original dll loaded from plugin folder");
                ...
                {
                    if (!_passThruMode)
                        LOG_INFO("VScaler working as wininet.dll, wininet-original.dll loaded");
                ...
                if (originalModule != nullptr && !_passThruMode)
                    LOG_INFO("VScaler working as wininet.dll, system dll loaded");
            ...
            {
                if (!_passThruMode)
                    LOG_ERROR("VScaler can't find original wininet.dll!");
            }
        ...
        if (lCaseFilename == "dbghelp.dll")
        {
            do
            {
                ...
                {
                    if (!_passThruMode)
                        LOG_INFO("VScaler working as dbghelp.dll, original dll loaded from plugin folder");
                ...
                {
                    if (!_passThruMode)
                        LOG_INFO("VScaler working as dbghelp.dll, dbghelp-original.dll loaded");
                ...
                if (originalModule != nullptr && !_passThruMode)
                    LOG_INFO("VScaler working as dbghelp.dll, system dll loaded");
            ...
            {
                if (!_passThruMode)
                    LOG_ERROR("VScaler can't find original dbghelp.dll!");
            }
        ...
        // vscaler.dll
        if (lCaseFilename == "vscaler.dll")
        {
            if (!_passThruMode)
                LOG_INFO("VScaler working as VScaler.dll");

            // quick hack for testing
            originalModule = dllModule;

            dllNames.push_back("vscaler.dll");
            dllNames.push_back("vscaler");
            dllNamesW.push_back(L"vscaler.dll");
            dllNamesW.push_back(L"vscaler");
        ...
        // vscaler.asi
        if (lCaseFilename == "vscaler.asi")
        {
            if (!_passThruMode)
                LOG_INFO("VScaler working as VScaler.asi");

            // quick hack for testing
            originalModule = dllModule;

            dllNames.push_back("vscaler.asi");
            dllNames.push_back("vscaler");
            dllNamesW.push_back(L"vscaler.asi");
            dllNamesW.push_back(L"vscaler");
        ...
        if (lCaseFilename == "winhttp.dll")
        {
            do
            {
                ...
                {
                    if (!_passThruMode)
                        LOG_INFO("VScaler working as winhttp.dll, original dll loaded from plugin folder");
                ...
                {
                    if (!_passThruMode)
                        LOG_INFO("VScaler working as winhttp.dll, winhttp-original.dll loaded");
                ...
                if (originalModule != nullptr && !_passThruMode)
                    LOG_INFO("VScaler working as winhttp.dll, system dll loaded");
            ...
            {
                if (!_passThruMode)
                    LOG_ERROR("VScaler can't find original winhttp.dll!");
            }
        ...
        if (lCaseFilename == "dxgi.dll")
        {
            do
            {
                ...
                {
                    if (!_passThruMode)
                        LOG_INFO("VScaler working as dxgi.dll, original dll loaded from plugin folder");
                ...
                {
                    if (!_passThruMode)
                        LOG_INFO("VScaler working as dxgi.dll, dxgi-original.dll loaded");
                ...
                if (originalModule != nullptr && !_passThruMode)
                    LOG_INFO("VScaler working as dxgi.dll, system dll loaded");
            ...
            {
                if (!_passThruMode)
                    LOG_ERROR("VScaler can't find original dxgi.dll!");
            }
        ...
        if (lCaseFilename == "d3d12.dll")
        {
            do
            {
                ...
                {
                    if (!_passThruMode)
                        LOG_INFO("VScaler working as d3d12.dll, original dll loaded from plugin folder");
                ...
                {
                    if (!_passThruMode)
                        LOG_INFO("VScaler working as d3d12.dll, d3d12-original.dll loaded");
                ...
                if (originalModule != nullptr && !_passThruMode)
                    LOG_INFO("VScaler working as d3d12.dll, system dll loaded");
            ...
            {
                if (!_passThruMode)
                    LOG_ERROR("VScaler can't find original d3d12.dll!");
            }
    ...
    if (d3d12AgilityModule == nullptr && State::Instance().gameQuirks & GameQuirk::LoadD3D12Manually)
    {
        ...
        {
            path = optiDllPath / L"D3D12_VScaler" / L"D3D12Core.dll";
            d3d12AgilityModule = NtdllProxy::LoadLibraryExW_Ldr(path.c_str(), NULL, 0);
        }
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD ul_reason_for_call, LPVOID lpReserved)
{
    ...
    case DLL_PROCESS_ATTACH:
    {
        ...
        {
            Config::Instance()->MainDllPath.set_volatile_value(L"VScaler");
        }
        ...
        spdlog::warn("---------------------------------");
        spdlog::warn("VScaler is freely downloadable from");
        spdlog::warn("GitHub : https://github.com/CRTYPUBG/vscaler/releases");
